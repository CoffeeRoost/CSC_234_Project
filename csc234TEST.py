...
TEN_THOUSAND_PI = "314159265358979323846264338327"#actual 10000 later
...
def encrypt_byte_array(byte_array, key, hypercube_length, square_length, num_dimensions):
    ...
    rotated_index_cube = apply_rotations_to_index_cube(index_cube, key, hypercube_length, square_length, num_dimensions)
...
def decrypt_hypercube(encrypted_cube, key, hypercube_length, square_length, num_dimensions):
    ...
    index_cube = create_index_cube(hypercube_length, num_dimensions)
    rotated_index_cube = reverse_rotations_to_index_cube(index_cube, key, hypercube_length, square_length, num_dimensions)
...
def unpad_with_pi(data, original_length):
    """Removes padding from the data, restoring it to its original length."""
...
def save_encrypted_cube(encrypted_cube, filename="encrypted_cube.bin"):
    ...
    with open(filename, "wb") as f:
        f.write(bytes(encrypted_cube.flatten()))
...
def load_encrypted_cube(filename="encrypted_cube.bin"):
    ...
    with open(filename, "rb") as f:
        data = f.read()
    encrypted_cube = np.frombuffer(data, dtype=np.uint8)
...
if deen:
    ...
    original_length = len(original_byte_array)
    #key_size = (hypercube_length**num_dimensions) * num_dimensions
    key = mykey
...
# Load Huffman tree
    ...
    unpadded_bytearray = unpad_with_pi(compressed_data,original_length)

    binary_string = bytearray_to_binary_string(unpadded_bytearray, padding)
    ...
    # Handle the case where unCover() returns -1 (not found)
    if pos == -1:
        print("Error: File size delimiter not found in decrypted data.")
        fileSize = -1  # Or handle the error as appropriate
    else:
        try:
